  ...
  <section class="max-w-6xl mx-auto px-6 py-16">
    ...
    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
      ...
      <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
        ...
        <form action="{{ route('articles.index') }}" method="GET" class="flex w-full sm:w-auto">
          <input class="w-full sm:w-64 rounded-l-lg border-gray-300 focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 text-sm py-2.5 px-3"
                 name="search"
                 placeholder="Search articles..."
                 value="{{ request('search') }}">
          <button class="bg-gray-900 hover:bg-gray-800 text-white px-4 py-2.5 text-sm font-medium rounded-r-lg transition-colors border border-transparent"
                  type="submit">
            Search
          </button>
        </form>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <article class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col h-full border border-gray-100">
          ...
          <div class="p-5 flex flex-col flex-grow">
            <div class="text-xs font-semibold tracking-wide text-teal-600 uppercase mb-2">
              By <a href="{{ route('users.userProfile', $article->user->id) }}" class="hover:underline">{{ $article->user->name }}</a>
            </div>
            ...
            <div class="pt-4 mt-auto border-t border-gray-100 flex items-center justify-between">
              ...
              <a href="{{ route('articles.show', $article) }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-gray-900 hover:text-teal-600 transition-colors">
                Read More
              ...
              <div class="flex items-center gap-3">
                @can('update', $article)
                  <a href="{{ route('articles.edit', $article) }}" class="text-xs font-medium text-amber-500 hover:text-amber-600 transition">
                    Edit
                  </a>
                @endcan

                @can('delete', $article)
                  <form action="{{ route('articles.destroy', $article) }}" method="POST" class="inline m-0" onsubmit="return confirm('Are you sure you want to delete this article?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-xs font-medium text-red-500 hover:text-red-600 transition">
                      Delete
                    </button>
                  </form>
                @endcan
      ...
      @empty
        <div class="col-span-full py-16 text-center bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            @can('create', App\Models\Article::class)
                <a href="{{ route('articles.create') }}" class="text-teal-600 font-medium hover:underline">Write the first article</a>
            @endcan
        </div>
      @endforelse
